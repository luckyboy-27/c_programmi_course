...
/* the main function works with input only everything else is handled by a function */
int main() {
    /* initialize variables */ 
    int choice='\0'; first=NULL;
    while(choice!='Q') {
    choice=menu(); 
    switch (choice) {
        case 'S': 
...
void delrec(void) {
    ...
    /* 'current' holds the record to delete */ /* is it the first record? */ 
    if(previous==NULL) {
        first=current->next;
